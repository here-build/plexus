...
> {
  private subDependencies = new Map<DependencyId, SubPlexus<any, ParentPlexus>>();
  ...
  constructor(
    ...
    rootPlexus['globalDependencyRegistry'].set(registryKey, { doc, plexus: this as any });
  ...
  private async loadSubDependencies(depVersions: Record<DependencyId, DependencyVersion>) {
    ...
    await Promise.all(
      deps.map(async ([depId, requestedVersion]) => {
        ...
        const resolvedVersion = depMetadata.get(YJS_GLOBALS.metadataMapFields.version) as DependencyVersion || requestedVersion;
   ...
   */
  getResolvedVersionInfo(): { requested: DependencyVersion; resolved: DependencyVersion } {
    return {
