...
import { DependencyId, Plexus } from "../plexus.js";
...
describe("Clone from dependency node", () => {
  ...
  it("produces an editable clone and materializes it into root", async () => {
    ...
    const depRoot = await plexus.addDependency<Container>("dep" as DependencyId, "latest");
    const depC = new Container([depContainerId, await plexus.fetchDependency("dep", "latest")]);
