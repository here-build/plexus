...
import { YJS_GLOBALS } from "@dappsnap/plexus"; // For packages that use plexus, ProjectId should be string
...
// Entity cache
const documentEntityCaches = new DefaultedWeakMap<Y.Doc, Map<string, WeakRef<ModelPattern>>>(
...
export function buildModelClass<T extends RecordSchemaInput | ModelPattern>(
    ...
    const proxy = new Proxy(
        ...
        Object.entries(schema).map(([key, type]) => {
          switch (type) {
            ...
            case "set": {
              ...
              return [key, setProxy as any as Set<AllowedYJSValue>];
            }
            default:
              return [key, null];
          }
      ...
      ),
      {
        get(target, key) {
          ...
          if (typeof key === "string" && Object.hasOwn(schema, key)) {
            ...
            return schema[key] === "val" || schema[key] === "child-val"
              ? deref(yprojectObjectInstanceFields.get(key) as AllowedYValue)
              : target[key];
          }
        ...
        set(target, elementKey, value) {
          ...
          if (projectId !== docProjectId) {
            console.warn(
              `cannot set property ${elementKey.toString()} of ${typeName} as it's readonly dependency reference`
            );
          ...
          if (typeof elementKey === "string") {
            ...
            if (keyType === "val" || keyType === "child-val") {
              ...
              yprojectObjectInstanceFields.set(elementKey, maybeReference(value, projectId, doc));
              return true;
            }
  ...
  const ModelConstructor = Object.defineProperties(
    function ModelConstructor(this: any, initialState: ReadonlyDeep<StrictRecordSchema<T>>) {
      ...
      const ephemeralModel = new Proxy(
        Object.fromEntries(
          Object.entries(schema).map(([key, type]) => {
            ...
            switch (type) {
              ...
              case "child-list": {
                ...
                return [key, ephemeralListProxy as Array<AllowedYJSValue>];
              }
              case "set":
              case "child-set": {
