/**
 * Exemplary plexus-do host for the xyflow showcase.
 *
 * The Worker names the object and plants the Flow seed. That is the first
 * writer — not an authority. Browsers only `connect`.
 */
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (action === "seed" && request.method === "POST") {
      ...
      await stub.seed(docId, body.byteLength > 0 ? body : encodeFlowSeed(docId));
...
async function ensureNamed(stub: DurableObjectStub<DocLeader>, docId: string): Promise<void> {
  ...
  await stub.seed(docId, encodeFlowSeed(docId));
}
