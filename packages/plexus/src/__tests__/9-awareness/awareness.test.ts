...
describe("PlexusAwareness: peer sync", () => {
  ...
  it("field update syncs independently", () => {
    ...
    const cursorCid = PlexusAwareness.channelId(awA.clientID, 1);
...
describe("PlexusAwareness: heartbeat and timeout", () => {
  ...
  it("field channels have independent clocks from channel 0", () => {
    ...
    const nameCid = PlexusAwareness.channelId(aw.clientID, 1);
  ...
  it("_removePeer cleans up all channels for a peer", () => {
    ...
    const cursorCid = PlexusAwareness.channelId(awA.clientID, 1);
    const nameCid = PlexusAwareness.channelId(awA.clientID, 2);
...
describe("PlexusAwareness: optional clientId + multi-instance per doc", () => {
  it("constructor(doc, clientId) uses the explicit base, not doc.clientID", () => {
    const doc = new Y.Doc();
    const custom = 42_001;
    const aw = new PlexusAwareness(doc, custom);
    expect(aw.clientID).toBe(custom);
    expect(aw.clientID).not.toBe(doc.clientID);
    aw.setField("role", "expectation");
    expect(aw.getField("role")).toBe("expectation");
    expect(aw.states.has(custom)).toBe(true);
    expect(aw.states.has(doc.clientID)).toBe(false);
    doc.destroy();
  });

  it("constructor(doc, { clientId }) options form matches number form", () => {
    const doc = new Y.Doc();
    const aw = new PlexusAwareness(doc, { clientId: 99_002 });
    expect(aw.clientID).toBe(99_002);
    doc.destroy();
  });

  it("rejects non-regular clientIds (channel packing would collide)", () => {
    const doc = new Y.Doc();
    const liminal = 2 ** 51; // LIMINAL_BASE
    expect(() => new PlexusAwareness(doc, liminal)).toThrow(/regular-range/);
    doc.destroy();
  });
