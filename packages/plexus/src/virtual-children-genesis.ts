...
function materializeVirtualStruct(owner: PlexusModel, fieldName: string, value: Y.AbstractType<any>): void {
  ...
  // genesisApplyUpdate keeps the transaction local (restoring the flag applyUpdate
  // flips, even when there's no active transaction) so the doc's clientId is never
  // rerolled out of its partition.
  genesisApplyUpdate(doc, diff);
}
