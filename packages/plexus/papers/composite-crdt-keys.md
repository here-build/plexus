...
**C1. Dangling references.** When a referenced entity is deleted, keys containing it become partially unresolvable. The application must detect and handle dangling references — the serialized string persists in the map even after the entity is gone. For set keys, partial resolution (some elements dangling) requires careful handling to avoid collision with legitimately null-containing keys. This mirrors the dangling foreign key problem in relational databases, with the same remediation options: ignore, cascade delete, or tombstone.
