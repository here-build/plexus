...
import type { AllowedKeyPrimitive, AllowedPrimitive, AllowedYJSKeyValue, AllowedYJSValue } from "./proxy-runtime-types.js";
...
// Set members are keys (dedup by member equality) → identity-bearing only, no bytes.
type DiscriminateChildSet<T extends Set<AllowedYJSKeyValue>, Parent extends PlexusModel> =
  T extends Set<infer V>
    ? CheckParentConstraint<V, T, Parent> extends never
      ? Set<AllowedKeyPrimitive | PlexusModel<Parent>>
      : T
    : never;
...
export interface SetDecorator {
  <Model extends PlexusModel, V extends AllowedYJSKeyValue, Struct extends Set<V>>(
    target: ClassAccessorDecoratorTarget<Model, Struct>,
    context: ClassAccessorDecoratorContext<Model, Struct> & { name: string },
  ): ClassAccessorDecoratorResult<Model, Struct>;
  declare<Out extends Set<AllowedYJSKeyValue>, In extends Out>(): <Model extends PlexusModel>(
    target: ClassAccessorDecoratorTarget<Model, Out>,
    context: ClassAccessorDecoratorContext<Model, Out> & { name: string },
  ) => DeclareResult<Out, In, Model>;
}
...
// Map keys are addressed by equality → identity-bearing only, no bytes. A
// `Map<Uint8Array, …>` extracts to `never` here, making the key untypeable.
type MapKey<T extends Map<any, any>> =
  T extends Map<infer K, any>
    ?
        | (Extract<K, Set<AllowedYJSKeyValue>> extends Set<AllowedYJSKeyValue> ? K : never)
        | (Extract<K, Array<AllowedYJSKeyValue>> extends Array<AllowedYJSKeyValue> ? K : never)
        | (Extract<K, AllowedYJSKeyValue> extends AllowedYJSKeyValue ? K : never)
    : never;
// Map values are payload at a known key → bytes allowed.
