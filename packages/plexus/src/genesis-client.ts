...
/**
 * Apply a genesis update without letting Yjs reroll the doc's own clientId.
 *
 * `applyUpdate` forces `transaction.local = false`; at cleanup that makes the doc's own
 * clientId look like a foreign client is using it, so Yjs reassigns it
 * (`Transaction.js:357`) — throwing it out of its Plexus partition. Wrapping in
 * `doc.transact` (which REUSES an active transaction, or opens a fresh local one when
 * there's none) and restoring `local` after the flip keeps genesis "local", so the
 * clientId is never rerolled — in either case. Genesis is a deterministic intra-doc
 * scaffold, not a remote update.
 */
export function genesisApplyUpdate(doc: Y.Doc, update: Uint8Array): void {
  doc.transact((txn) => {
    const savedLocal = txn.local;
    Y.applyUpdate(doc, update);
    txn.local = savedLocal;
  });
}
