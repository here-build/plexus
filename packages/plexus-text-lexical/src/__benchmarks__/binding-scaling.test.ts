/**
 * Lexical binding: remote 1-char insert timing + E1 firing decomposition.
...
// Binding path is heavier than model-only: keep ladder smaller so CI stays <2min.
...
function editorText(ed: ReturnType<typeof makeEditor>): string {
  return ed.getEditorState().read(() => $getRoot().getTextContent());
}

describe("plexus-text-lexical bench — binding W1 remote mid insert", () => {
  it("measures t_apply + nodesScanned + E1 firings across sizes", () => {
    ...
    for (const N of SIZES) {
      ...
      const unB = bindLexical(edB, B.root, B.doc);
      ...
      for (let i = 0; i < measured; i++) {
        ...
        const { delta } = withCounterWindow(() => {
          ...
          // N1 drains reaction sync via doc.on("update")
        ...
        if (editorText(edB) !== toText(B.root)) faithfulness = false;
      }
    ...
    console.log("\n=== Lexical binding W1 remote mid insert ===");
    // eslint-disable-next-line no-console
    console.table(rows);
    // eslint-disable-next-line no-console
    ...
    expect(rows.every((r) => r.faithful === 1)).to.equal(true);
    ...
    for (const r of e1Rows) {
      ...
      // N2: toText+segments once each per pull cycle (not 4×)
      expect(r.toText_med).to.be.at.most(2);
      expect(r.segments_med).to.be.at.most(2);
    }
