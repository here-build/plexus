...
There is no logger API and no env-var gating. Every PlexusError schedules an unconditional
async `console.error` (via `setTimeout`, next tick) on construction — see `src/errors.ts`.
Logging is independent of throwing; the error throws regardless.
